...
func _on_roam_timeout() -> void:
	...
	sound_player = SoundManager.play_spatial_sfx(load("res://sfx/entity/passive/pig_snort_" + str(random_sound) + ".wav"), self, 1.0)
	sound_player.max_distance = 100
		
func _on_death():
	if last_damage_source and last_damage_source.is_in_group("player"):  
		run_loot_table()
	sound_player = SoundManager.play_spatial_sfx(load("res://sfx/entity/passive/pig_death.wav"), self, 0.5)
	velocity = Vector2.ZERO
	super()
