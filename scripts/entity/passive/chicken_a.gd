...
func _on_roam_timeout() -> void:
	...
	if roll < idle_chance:
		...
		current_state = State.ACTION_1
		play_animation("side_action_1", Vector2.ZERO)
	...
	var random_sound = randi_range(1, 2)
	if random_sound == 1:
		sound_player = SoundManager.play_spatial_sfx(load("res://sfx/entity/passive/chicken_cluck_1.wav"), self, 1.0)
	else:
		sound_player = SoundManager.play_spatial_sfx(load("res://sfx/entity/passive/chicken_bawk_1.wav"), self, 1.0)
